...
/**
 * Read from FujiNet app key store.
 *
 * Note: Fails silently.
 */
void read_settings(void) {
  ...
  if (dstats == 1 /* operation complete (no errors) */) {
    ...
    if (OS.dcb.dstats == 1 /* operation complete (no errors) */) {
      /* FIXME: pull data from app_key_data.read.value */
...
/**
 * Write settings to FujiNet app key store.
 *
...
unsigned char write_settings(void) {
  ...
  if (dstats == 1 /* operation complete (no errors) */) {
    /* FIXME: write data to app_key_data.write.value */
