...
{ 
  ...
  #ifdef USE_VICE_EMULATOR  
  strcpy(appkey.payload.write.value,OPEN_APPKEY_PREFIX);
  ...
  itoa(app_id, &appkey.payload.write.value[strlen(appkey.payload.write.value)], 10);
  
  strcat(appkey.payload.write.value, OPEN_APPKEY_DELIM);
  itoa(key_id, &appkey.payload.write.value[strlen(appkey.payload.write.value)], 10);
  ...
  appkey_error = cbm_open(LFN,DEV,open_mode,appkey.payload.write.value);
...
unsigned char write_appkey(unsigned int creator_id, unsigned char app_id, unsigned char key_id, const char *data)
{
  ...
  appkey.payload.read.reserved = 0x00;
